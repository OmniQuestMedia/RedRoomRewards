...
- **Evidence**: JWT-based auth with UserRole enum
  (ADMIN, USER, MODEL, QUEUE_SERVICE, SYSTEM)
...
4. **src/api/events.controller.ts**
   - Updated PostEventRequest interface to include merchantId and
     correlationId (lines 24-25)
   ...
   - Added structured logging throughout postEvent()
     (lines 68, 82-90, 99-111, 115-127, 130-141)
   - Added metrics counters for all outcomes
     (lines 65-67, 76-80, 100-104, 116-120, 131-135)
...
1. **src/api/events.controller.spec.ts**
...
2. **src/services/support.service.spec.ts** (NEW)
...
Request → EventsController.postEvent() [line 64]
  ...
  ↓ Include in all logs
  ↓ [line 68, 82, 99, 115, 130] MetricsLogger.logRedactedIngest({
      correlationId, ... })
...
✓ Receipt lookup returns ONLY: correlationId, merchantId, eventId,
...
Note: The SupportService is fully implemented and tested at the unit level.
It just needs to be exposed via an HTTP endpoint with proper auth middleware.
